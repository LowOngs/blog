...
#!/usr/bin/env node
'use strict';
// review-fetch-official: 공식 API(1안)로 평점/투표/히스토그램을 수집해 *-ratings-next.json 업서트
...
function normalizeHistogram(h) {
  ...
  // Invariants: '1'..'5' 키가 항상 존재, 숫자/NaN 방지
...
function pickReviewKey(doc) {
  // What: posts JSON에서 provider/storeId/reviewId를 안전하게 추출
  // Why: 필드 확정 전 과도기(다중 후보 필드) 호환
  // I/O: R(content/posts/*.json), W(없음)
  // Invariants: storeId 없으면 fetch 스킵(외부 호출 금지)
...
async function fetchGooglePlay({ storeId }) {
  ...
  // 반환 스키마는 fetchCommon()의 expected 형태를 따라야 함
  return null;
}
...
async function fetchCommon(provider, key) {
  ...
  // Why: 단일 진입점으로 호출량/리트라이/타임아웃 정책을 일괄 적용하기 위함
...
function upsertNextEntry(nextMap, baselineMap, slug, provider, storeId, fetched) {
  ...
  // Invariants: fetched가 없으면 next를 변경하지 않음(불필요 오염 방지)
...
function upsertSources(sourcesMap, slug, provider, storeId, url) {
  // What: review-sources.json에 공식 출처를 최소 1개 기록
  // Why: AIO/신뢰/추적(나중에 검증/갱신 루프) 기반 마련
  // I/O: R/W(content/reviews/review-sources.json)
  // Invariants: url 없으면 sources는 건드리지 않음(불필요 오염 방지)
  if (!url) return false;
...
function buildProviderUrl(provider, storeId) {
  // What: 공식 페이지 URL(가능한 경우) 생성
  // Why: sources 기록에 활용(공식 API가 아니라도 공식 페이지 링크는 유효)
  // I/O: R(provider/storeId), W(없음)
  ...
  if (provider === 'trustpilot') {
    // business unit slug/id (형식은 운영에서 확정)
...
async function main() {
  ...
  for (const k of candidates) {
    ...
    if (upsertSources(sources.bySlug, slug, provider, storeId, url)) {
      sourcesAdded += 1;
    }
  }
